...
class Playlist:
    ...
    def _is_spotify(self):
        ...
        match = re.match(
            r"^(https://)?open.spotify.com/(user/.*/)?playlist/.*?$", self.URL
        )
        if match:
            self.type = "spotify"
    ...
    def _is_jiosaavn(self):
        ...
        """
        match = re.findall(
            r"^(https://)?(www.)?(jio)?saavn.com/(s/playlist|featured)/.*?", self.URL
        )
        if len(match):
            self.type = "jiosaavn"
    ...
    def get_data(self):
        ...
        data, name = self._get_data()
        logger.info(
            "{}: {} {}".format(name, len(data), "song" if len(data) < 2 else "songs")
        )
